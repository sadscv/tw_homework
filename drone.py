...
class Drone(object):
    def __init__(self, filepath=None):
        ...
        if filepath:
            try:
                ...
                logger.error('file not found, check the filename and path<==== line:{}'.format(self.line_idx))
                sys.exit(1)
        ...
        with open(filepath, 'r') as f:
            ...
            for line in f:
                ...
                if self.working and self.valid_signal(line):  # if self.working, add current position to paths
                    logger.info('validating:{},result:{}<==== line:{}'.format(line.strip(), True, self.line_idx))
                    self.paths.append(tuple(self.pos))
                else:  # if self.working==False, add 'NA' to paths
                    logger.info('validating:{},result:{}<==== line:{}'.format(line.strip(), False, self.line_idx))
    ...
    def valid_signal(self, signal):
        ...
        if len(sigList) == 4 and self.id is None:
            ...
            if sigList[0].isalnum() and all(self.is_digit(s, int_only=self.INT_ONLY) for s in sigList[1:]):
                self.id = sigList[0]
                self._fly([float(s) for s in sigList[1:]])
                self.debut = [float(s) for s in sigList[1:]]
                return True
            else:  # Case 1.3 if not matched,return False
                logger.error('name should compose by integers and alphabets <==== line:{}'.format(self.line_idx))
        ...
        # Case2: drone is not first seen in aera, and length of singal == 7
            ...
            if sigList[0] == self.id and all(self.is_digit(s, int_only=self.INT_ONLY) for s in sigList[1:]):
                prev_pos = [float(i) for i in sigList[1:4]]
                # if previous position matched, add current offset as drone's next step
                is_match = all(prev_pos[i] == self.pos[i] for i in range(len(self.pos)))
                if is_match:
        ...
        # some bad Cases
                ...
                logger.error('parameter is invalid <==== line:{}'.format(self.line_idx))
            else:
                logger.error('name is not matched line:{}'.format(self.line_idx))
        elif len(sigList) == 4 and self.id:
            logger.error('length is 4 but not first appear in area <==== line:{}'.format(self.line_idx))
        elif len(sigList) == 7 and self.id is None:
            logger.error('length is 7 but first apprear <==== line:{}'.format(self.line_idx))
        else:
            logger.error('length is invalid <==== line:{}'.format(self.line_idx))
        return False
    ...
    def check(self, line):
        ...
        results = [self.id, line]
        for i in self.paths[int(line)]:
            if int(i) == float(i):
                results.append(int(i))
            else:
                results.append(float(i))

        logger.info('Check line: result:{} <==== line:{}'.format(results, line))
        return ' '.join(map(str, results))

    def error_time(self):
        for i in range(len(self.paths)):
            if self.paths[i] == ('NA', 'NA', 'NA'):
                return i
        return -1
